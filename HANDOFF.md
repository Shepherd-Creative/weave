...
**Status**: **Wave 1 is CLOSED.** [PR #9](https://github.com/Shepherd-Creative/weave/pull/9) is **MERGED**. The next action is **Wave 2, in a fresh branch and worktree** — beginning once the [issue #10](https://github.com/Shepherd-Creative/weave/issues/10) record correction carrying this text has landed on `main`.
...
PR #9 is merged. Wave 1's exit gate is closed. **Do not push to `feature/primitive-portfolio-wave-1`, and do not wait on a merge decision — it has been made.** Wave 2 starts in its own branch and worktree cut from current `main`, **once the [issue #10](https://github.com/Shepherd-Creative/weave/issues/10) record correction that carries this text has landed there**. That is the one Wave 1 item still in flight, and the local plan states the same gate.
...
Measured on this branch against the **final** checker, before this correction merged. Every run below is a throwaway copy driven through the checker's `HANDOFF_PATH` override, so the canonical file was never modified and there was no restore to get wrong — the safest form, and the one to prefer when the checker supports it.

| Probe | Result |
|---|---|
| **Falsification 1** — rewrite the header `**Status**:` line back to a pre-merge wording | **exit 1**, `69 checks, 68 passed, 1 failed`: `B10 header Status line declares Wave 1 CLOSED and PR #9 MERGED` |
| **Falsification 2** — relabel the evidence-table tree row with the equality an earlier draft shipped | **exit 1**, `69 checks, 67 passed, **2** failed`: `B9b` **and** `E1` |
| **Falsification 3** — re-introduce, in the **Exit gate** section, the affirmative sentence that actually shipped, leaving the corrected sentence in place | **exit 1**, `69 checks, 68 passed, 1 failed`: `E1`, which printed the offending sentence and named its section |
| **Bypass probe** — an affirmative preceded by a denial in a *neighbouring* sentence (`… old draft! <affirmative>.`) | **exit 1**, `69 checks, 68 passed, 1 failed`: `E1` — the bypass is refused |
| **Control probe** — blank every pattern in the sweep's own list | **exit 1**: `E0b CONTROL: sweep encountered and suppressed 0 …` — the control collapses when the sweep stops matching |
| Final | `node validate-closeout.mjs` → **exit 0**, `69 checks, 69 passed, 0 failed` |

**Falsification 2 flips two checks, not one, and that is recorded rather than tidied.** An earlier draft of this table claimed one. It was measured before `E1` existed; adding `E1` gave the same sabotage a second detector, and re-measuring against the final checker is what surfaced the discrepancy. **A recorded result is only true of the checker that produced it** — the same class of staleness this whole document exists to correct, caught here in the evidence rather than in the claims.

**The bypass and control probes exist because a reviewer constructed them.** The sweep originally split sentences on `.` and newline only, so a denial in an adjacent clause would have whitelisted an affirmative next to it; boundaries now include `!`, `?` and `;`. And its non-vacuity control originally counted mentions with a free-standing regex over the whole file, which would have kept passing from historical text while the sweep itself matched nothing — it now counts only what the sweep actually encountered and suppressed, which is why blanking the patterns turns it red.

**Falsification 3 exists because falsifications 1 and 2 both passed while the document was still wrong.** The affirmative form of the conflation — a sentence stating outright that the approved tree and the merged tree are one tree — survived in the **Exit gate** section and was pushed. `B9b` was scoped to the current-state block, so it could not see it; every check that merely looks for `32f31d9` stayed green because the sha was correct — only the sentence was false. **A guard that inspects one section cannot speak for the others**, and a document-wide `grep` was not an option either, because the corrected text has to quote the falsehood in order to deny it. `E1` therefore reads every current section, and judges each match **in context**: a mention inside a sentence carrying an explicit denial is allowed, a bare affirmative is not. Its own non-vacuity control asserts that quoted-and-denied mentions are still being seen, so "zero offending" cannot mean "searched nothing".
...
**Two independent read-only reviews returned blocking findings, and both were right.** The first, an adversarial review of an earlier draft: this table had said *"reviewed tree == merged tree"*, but the approved head `3419bc7` has tree `9bd453d…`, so the merged tree is emphatically **not** the reviewed tree. It also found that the CI-run check could pass from the prose that *describes* the evidence rather than from the evidence row, that a blanket "everything below is history" banner would sweep up the Wave 2 instructions written for the next reader, and that a base-ref sentence pinned a sha this correction's own merge would invalidate.

**The second found that the first fix had been applied incompletely — and that the checker had certified it anyway.** The identical false affirmative survived in the **Exit gate** section and was pushed in a head the checker reported green. That is a validator failure, not merely an editing slip: the guard had been written against the place the fault was first found rather than against the claim itself. The fix is `E1` above, plus the discipline it encodes — when a corrected claim can recur anywhere, the guard's scope must be every current record, not the one section that happened to fail.

**A third review then found that `E1` itself could be evaded**, and that its non-vacuity control was not derived from the sweep it claimed to control; it also caught that this document and the local plan disagreed about whether Wave 2 could begin, and that falsification 2's recorded result no longer matched the checker. All are fixed above, and the sweep now covers the plan's closing record and the corrective PR's own body as well — the PR body being where one of these falsehoods had already shipped once. Every review's findings are resolved in the text above. **No re-review is claimed for any text later than the review that examined it**, so nothing here should be read as carrying an approval.
...
- [ ] **Start Wave 2, in its own branch and worktree cut from current `main`.** This is the next action. The only Wave 1 item still in flight is the [issue #10](https://github.com/Shepherd-Creative/weave/issues/10) record correction that carries this text; **Wave 2 begins once that has landed on `main`**, which is the same gate the local plan states. Nothing else in Wave 1 blocks it.
